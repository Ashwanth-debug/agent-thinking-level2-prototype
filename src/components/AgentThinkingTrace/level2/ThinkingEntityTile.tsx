...
export interface ThinkingTileModel {
  ...
  image?: string;
  /** Google Places id, when the entity resolved to one — carried on the model
   *  for consumers that still key off it, but no longer feeds image resolution
   *  (see EnrichedImage: trace photo -> local placeholder only, no external
   *  Places lookup). */
  placeId?: string;
  ...
  return (
    ...
    >
      ...
      {/* The media frame is ALWAYS rendered once a tile has any image source,
          ...
          finally arrives. The resolver is the project's own trace photo,
          falling through to a local placeholder — no external image API — so
