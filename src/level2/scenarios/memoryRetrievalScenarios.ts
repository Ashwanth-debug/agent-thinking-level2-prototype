...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   WHY FIXTURES, NOT REAL TRACES: real memory hits observed in practice were
   too sparse and repetitive (typically a single location fact) to make the
   multi-signal, multi-domain demonstration this scenario exists to show.
   Every entry here is `source: 'fixture'` — clearly marked as such, never
   presented as recorded output — and the registry (see registry.ts) never
   tries a real trace for this pool. `fx-memory-location` mirrors the one
   signal shape actually observed in practice (a location fact) so the demo
   includes an example grounded in real evidence, without pretending it IS a
   real trace.
