...
const ROUTE_MAP: Level2Scenario[] = [
  scenario(
    ...
    [
      ...
      v('rt1-b', '18 minutes saved going via Mysuru Road', 'route', {
        ...
        // Demo geometry — public city-level coordinates only, endpoints
        // honesty preserved (no road polyline is implied).
        geo: {
          geometry: 'endpoints',
          origin: { lat: 12.9716, lng: 77.5946, label: 'Bengaluru' },
          destination: { lat: 12.4244, lng: 75.7382, label: 'Madikeri' },
        },
        stage: 'summary',
      }),
  ...
  scenario(
    ...
    [v('rt2-a', 'Checking both routes at your actual departure time', 'route', { stops: [], eta: '48 min', distance: '39 km', savings: '12 minutes saved', origin: 'Indiranagar', destination: 'Kempegowda International', geo: { geometry: 'endpoints', origin: { lat: 12.9784, lng: 77.6408, label: 'Indiranagar' }, destination: { lat: 13.1986, lng: 77.7066, label: 'Airport' } }, stage: 'summary' })],
    {
      kind: 'route',
  ...
  scenario(
    ...
    [s('rt3-a', 'Grouping places by how far apart they are'), v('rt3-b', 'North and south split cleanly into two days', 'route', { stops: [], origin: 'Panjim', destination: 'Palolem', geo: { geometry: 'endpoints', origin: { lat: 15.4909, lng: 73.8278, label: 'Panjim' }, destination: { lat: 15.0100, lng: 74.0232, label: 'Palolem' } }, stage: 'route' })],
    {
      kind: 'route',
    ...
    [
      ...
      v('h1-b', 'Nandi Hills is the shortest drive', 'route', {
        ...
        geo: {
          geometry: 'endpoints',
          origin: { lat: 12.9716, lng: 77.5946, label: 'Bengaluru' },
          destination: { lat: 13.3702, lng: 77.6835, label: 'Nandi Hills' },
        },
        stage: 'summary',
      }),
