...
export function scheduleActualPasses(
  ...
  while (i < passes.length) {
    ...
    if (pass.traceTiming) {
      ...
      const end = Math.max(remap(pass.traceTiming.end), start + MIN_INTERPOLATED_MS);
    ...
    const windowEnd = Math.max(nextAnchor, windowStart + run.length * MIN_INTERPOLATED_MS);
    ...
    for (const p of run) {
      const duration = Math.max(MIN_INTERPOLATED_MS, (p.enterDuration + p.holdDuration + p.exitDuration) * scale);
